...
void Display(Node* root, int nivel)
{
	...
	if (root != 0)
	{
		Display(root->right, nivel + 1); //mergem cat putem in dreapta si crestem nivelul
		for (i = 0; i <= nivel; i++)
			printf("      ");
		printf("%d\n", root->data);
...
//Alocam memorie pentru noul nod si stabilim valorile pentru data,left_child,right_child.
	...
	if (root == NULL)
	{
		root = (Node*)malloc(sizeof(Node)); //alocam memorie
		root->data = data;
		root->left = root->right = NULL; //la copii punem null ptc sunt primii
	}
...
void preorder(Node* root)//RSD
{
	...
	if (root != NULL) //afisezi ce e in el si apoi te duci si in stanga si in drepata
	{
